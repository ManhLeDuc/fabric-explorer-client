...
describe('Charts', () => {
	describe('Operations', () => {
		...
		test('blockPerHour', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/blocksByHour/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('blockPerMin', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/blocksByMinute/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('changeChannel', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/changeChannel/${channel}`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('dashStats', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/status/${channel}`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('peerStatus', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/peersStatus/${channel}`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('transactionByOrg', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/txByOrg/${channel}`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('transactionPerHour', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/txByHour/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('transactionPerMin', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/txByMinute/${channel}/1`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});
		...
		test('blockActivity', () => {
			nock(/\w*(\W)/g)
				.get(process.env.PUBLIC_URL + `/api/blockActivity/${channel}`)
				.reply(200, {
					rows: [{ test: 'rows' }]
				});

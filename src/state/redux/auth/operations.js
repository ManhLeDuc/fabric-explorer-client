...
const login = ({ user, password }, networkObj) => dispatch =>
	post(process.env.PUBLIC_URL + '/auth/login', { user, password, network: networkObj })
		.then(resp => {
...
const network = () => dispatch =>
	get(process.env.PUBLIC_URL + '/auth/networklist', {})
		.then(({ networkList }) => {
...
const register = user => dispatch =>
	post(process.env.PUBLIC_URL + '/api/register', { ...user })
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
...
const userlist = () => dispatch =>
	get(process.env.PUBLIC_URL + '/api/userlist')
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
					actions.getErroMessage(
...
const unregister = user => dispatch =>
	post(process.env.PUBLIC_URL + '/api/unregister', { ...user })
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
					actions.getErroMessage(
...
const logout = () => dispatch =>
	post(process.env.PUBLIC_URL + '/auth/logout', {})
		.then(resp => {
			console.log(resp);

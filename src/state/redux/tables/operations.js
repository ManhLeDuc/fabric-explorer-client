...
const blockList = channel => dispatch =>
	get(process.env.PUBLIC_URL + `/api/blockAndTxList/${channel}/0`)
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
...
const blockListSearch = (channel, query) => dispatch =>
	get(process.env.PUBLIC_URL + `/api/blockAndTxList/${channel}/0?${query}`)
		.then(resp => {
...
const chaincodeList = channel => dispatch =>
	get(process.env.PUBLIC_URL + `/api/chaincode/${channel}`)
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
					actions.getErroMessage(
...
const channels = () => dispatch =>
	get(process.env.PUBLIC_URL + '/api/channels/info')
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
					actions.getErroMessage(
...
const transaction = (channel, transactionId) => dispatch =>
	get(process.env.PUBLIC_URL + `/api/transaction/${channel}/${transactionId}`)
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
					actions.getErroMessage(
...
const transactionListSearch = (channel, query) => dispatch =>
	get(process.env.PUBLIC_URL + `/api/txList/${channel}/0/0?${query}`)
		.then(resp => {
...
const transactionList = channel => dispatch =>
	get(process.env.PUBLIC_URL + `/api/txList/${channel}/0/0/`)
		.then(resp => {
			if (resp.status === 500) {
				dispatch(
					actions.getErroMessage(
